...
function BookCard({
  ...
  return (
    <div
      className={`bg-white dark:bg-[#1A1D27] rounded-2xl p-4 shadow-sm border transition-all ${
        isComplete
      ...
      <div className="flex items-start gap-3">
        ...
        {!isComplete && book.status === "reading" && (
          <button
            ...
            className="flex-shrink-0 px-3 py-1.5 bg-blue-500 hover:bg-blue-600 text-white text-xs font-medium rounded-xl transition-colors"
          >
            기록
          </button>
        )}
        ...
        {/* 완독 배지 */}
        {isComplete && (
          <span className="flex-shrink-0 text-[10px] text-gray-400 dark:text-gray-500 font-medium">
            완독 ✓
          </span>
        )}
      </div>
...
function RecordPageModal({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md bg-white dark:bg-[#1A1D27] rounded-2xl p-6 shadow-xl"
      ...
      >
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
          현재 <span className="font-bold text-blue-500">{book.read_pages}p</span>{" "}
        ...
        <label className="block mb-4">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="flex-1 border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#22252F] rounded-xl px-3 py-2.5 text-sm text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-400"
            />
        ...
        {/* 미리보기 */}
        {newPages > 0 && (
          <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-950/20 dark:to-purple-950/20 rounded-xl p-3 mb-5 border border-blue-100 dark:border-blue-900/30">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="flex-1 py-2.5 bg-blue-500 hover:bg-blue-600 disabled:opacity-40 text-white rounded-xl text-sm font-medium transition-colors"
          >
            {saving ? "저장 중..." : "기록하기"}
          </button>
        </div>
...
function AddBookForm({ onAdd }: { onAdd: (book: Omit<Book, "id" | "user_id" | "read_pages" | "status" | "completed_at" | "created_at">) => Promise<void> }) {
  ...
  if (!open) {
    return (
      <button
        ...
        className="w-full py-3 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-2xl text-sm text-gray-400 dark:text-gray-500 hover:border-blue-300 hover:text-blue-500 transition-colors flex items-center justify-center gap-2"
      >
  ...
  return (
    <form
      ...
      className="bg-white dark:bg-[#1A1D27] rounded-2xl p-4 shadow-sm border border-blue-100 dark:border-blue-900/30"
    >
      ...
      <div className="space-y-3">
        ...
        <input
          ...
          required
          className="w-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#22252F] rounded-xl px-3 py-2.5 text-sm text-gray-800 dark:text-gray-100 placeholder-gray-300 dark:placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-400"
        />
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3">
        {filtered.length === 0 ? (
          ...
          filtered.map((book) => (
            <BookCard key={book.id} book={book} onRecordPage={setRecordingBook} />
          ))
        )}
      </div>
